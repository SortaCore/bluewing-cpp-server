...
bool relayserverinternal::client_messagehandler(std::shared_ptr<relayserver::client> client, lw_ui8 type, std::string_view messageP, bool blasted)
{
	...
	if (messagetypeid != 0 && messagetypeid != 9 && !client->connectRequestApproved)
	{
		...
		error->add("Dropping connecting client from IP %s for sending messages before connection was approved", addr);
...
std::shared_ptr<relayserver::channel> relayserver::createchannel(std::string_view channelName, std::shared_ptr<relayserver::client> master, bool hidden, bool autoclose)
{
	...
	{
		auto serverWriteLock = lock.createWriteLock();
		((lacewing::relayserverinternal *)internaltag)->channels.push_back(channel);
	}
...
/// <summary> Approves or sends a deny response to channel leave request. Pass empty string_view for deny reason if approving.
...
/// <param name="denyReason">	The deny reason. If empty, the channel is approved (if new channel name is legal).
/// 							If non-empty, channel join deny is sent, and channel is cleaned up as needed. </param>
